...
### Changed
- **Dependency Upper Bounds**: Added upper-bound constraints (`< next_major`) to core dependencies (`reportlab`, `mistletoe`, `requests`, `typer`, `pillow`, `pygments`, `charset-normalizer`) and optional `matplotlib` in `pyproject.toml` to protect downstream installs from future breaking changes.

## [0.6.1] - 2026-09-02

### Fixed
- **Section & List Layout Splitting**: Extended `KeepTogetherParts` to support `ListFlowable`, `Paragraph`, `BlockQuoteBar`, and `AdmonitionBox`, allowing content to start on the current page if initial items fit rather than pushing the entire block to a new page.
- **Consecutive Heading Orphan Prevention**: Grouped consecutive headings and bookmark anchors in `LayoutComposer` so parent headings are never stranded alone when subheadings break across page boundaries.
...
- **Running Header & Outline HTML Entity Unescaping**: Unescaped HTML/XML entities (such as `&amp;`) in heading bookmark titles and running section templates to prevent literal entity string displays in PDF viewer outline panels and page headers.
